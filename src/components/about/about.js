...
export default function About({ context }) {
    ...
    const [hovered, setHovered] = useState(convertArrayToObject(context?.pageContext?.edge?.about?.team?.map((_, i) => {
        return {
            key: i,
            state: false
        }
    ...
    const hoverOverHandler = async (i) => {
