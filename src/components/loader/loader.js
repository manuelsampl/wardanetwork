        ...
        <motion.div className="full-loader" variants={variants} initial={"initial"} animate={"animate"} transition={defaultTransition} style={{ backgroundColor: color, zIndex: '99999999999999999999' }}>
            {cnt >= -1 && !stop ?
                <>
                    ...
                    <GatsbyImage loading="eager" className="video-tablet" alt={images[cnt + 1].imageDesktop.altText} image={imageTablet} />
                    <GatsbyImage loading="eager" className="video-mobile" alt={images[cnt + 1].imageDesktop.altText} image={imageMobile} />
                </>
                : <></>}
        </motion.div >
    )
